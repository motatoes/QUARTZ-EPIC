classdef Vessel_Data < hgsetget
    ...
    properties
        ...
        segment_label; %vessel segment labels in the image
        od_location  ; %x and y cordinates of optic disc location
    end
    ...
    methods
        ...
        function update_image_lines(obj, h, repaint_all)
            ...
            if nargin < 2 || isempty(h) || ~ishandle(h)
                h = get_figure(obj);
                % If there is no handle, look for it 
                if isempty(h)
...
%                 set(findobj(ax, 'type', 'image'), 'CData', im2, 'CDataMapping', 'scaled');
    ...
    methods
        ...
        function remove_short_vessels(obj, min_diameters)
            ...
            iid_removed = find(inds_remove==1);
            seglab = obj.segment_label;
            maxlbl= max(max(seglab));
            
            for jj = 1:numel(iid_removed)
               ind = seglab == iid_removed(jj);
               seglab(ind) = 0;
            end
            for jj = 1:numel(iid_removed)
                ind = seglab > iid_removed(jj);
                seglab(ind) = seglab(ind)-1;
                maxlbl = maxlbl-1;
                indmax = seglab>maxlbl;
                seglab(indmax) = 0;
            end
            if(~isempty(iid_removed))
                obj.segment_label = seglab;
            end
        end
    ...
    methods
       ...
       function val = get.id(obj)
        ...
        function set.selected_vessel_ind(obj, val)
            ...
            % commented the update image lines because
            update_image_lines(obj);
        end
    end
    ...
    methods
        ...
        function set.od_location(obj, val)
                obj.od_location = val;
        end
        
        function val = get.od_location(obj)
                val = obj.od_location ;
        end
    ...
    methods (Static)
        function obj = loadobj(obj)
            if isstruct(obj) || isa(obj, 'Vessel_Data')
                ...
                new_obj.segment_label = obj.segment_label;
                new_obj.od_location = obj.od_location;
                % Return new object
                obj = new_obj;
            end
        end
    ...
    methods
        ...
        function obj = saveobj(obj)
            ...
            s.args         = obj.args;
            s.segment_label = obj.segment_label;
            s.od_location = obj.od_location;
            obj = s;
        end
    end
