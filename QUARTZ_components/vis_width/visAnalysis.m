function varargout = visAnalysis(varargin)
% visAnalysis Analyse retinal vessel diameters with a Graphical User Interface.
%
%      visAnalysis makes it possible to measure vessel diameters
%      automatically in a wide range of image types.  Simply type visAnalysis at
%      the MATLAB command prompt to run it.
%
% Copyright © 2011 Peter Bankhead.
% See the file : Copyright.m for further details.
%
% Last Modified by GUIDE v2.5 07-Nov-2013 10:53:47

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @visAnalysis_OpeningFcn, ...
    'gui_OutputFcn',  @visAnalysis_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before visAnalysis is made visible.
function visAnalysis_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to visAnalysis (see VARARGIN)
% Choose default command line output for visAnalysis
handles.output = hObject;
centerfig(handles.fig_QUARTZ_VESSEL_ANALYSIS);
if( isappdata(0,'vdobj') )
    vd = getappdata(0,'vdobj');
    
    handles.vessel_data  = vd.vessel_data;
    handles.settings = vd.settings;
    handles.vessel_data.settings = vd.settings;
    rmappdata(0,'vdobj');
    
    CustomizeToolBar(handles.fig_QUARTZ_VESSEL_ANALYSIS);
    set(handles.menu_file, 'Visible','Off');
    set(handles.menu_processors, 'Visible','Off');
    set(handles.menu_retina, 'Visible','Off');
    set(handles.menu_display, 'Visible','On');
    set(handles.menu_vessels, 'Visible','Off');
    set(handles.menu_copy, 'Visible','Off');
    set(handles.menu_image, 'Visible','Off');
    set(handles.menu_plot, 'Visible','Off');
    set(handles.menu_spacing, 'Visible','Off');
    set(handles.menu_double_buffer, 'Visible','Off');
    ...
    update_image_lines(handles.vessel_data, handles.axesImage);
    
    
        mouseX = handles.vessel_data.od_location(1);
        mouseY = handles.vessel_data.od_location(2);
        mR = 10;
        %[pointsX,pointsY] = circlePTS(mouseX,mouseY,mR,50);
        %plot(pointsX,pointsY,'Color','blue','LineWidth',mR);
        ...
        w = 10; h = 30;
        handles.od_loc.r1 = rectangle('Position',[mouseY-(w/2),mouseX-(h/2),w,h],'LineWidth',1, 'FaceColor' , 'yellow', 'tag' ,'OD_LOCALIZE');
        w=30;h=10;
        handles.od_loc.r2 = rectangle('Position',[mouseY-(w/2),mouseX-(h/2),w,h],'LineWidth',1, 'FaceColor' , 'yellow','tag' ,'OD_LOCALIZE');
        
        %handles.od_loc.r3 = rectangle('position',[mouseY-mR, mouseX-mR, 2*mR, 2*mR],'curvature',[1 1],'LineWidth',1, 'FaceColor' , 'blue','tag' ,'OD_LOCALIZE');
        vis = 'off';
        if(handles.settings.show_optic_disc)
            vis = 'on';
        end    
        set(handles.od_loc.r1, 'Visible' , vis);
        set(handles.od_loc.r2, 'Visible' , vis);
        %set(handles.od_loc.r3, 'Visible' , vis);
        
    set(handles.fig_QUARTZ_VESSEL_ANALYSIS, 'WindowButtonDownFcn'  , {@seedclick_short , handles});
...
function varargout = visAnalysis_OutputFcn(hObject, eventdata, handles) %#ok<*INUSL>
...
%axes(handles.axesImage);
% Update painted lines
 %set(handles.axesImage , 'DrawMode' , 'fast');
 %update_image_lines(handles.vessel_data, handles.axesImage);
...
if ~isempty(handles.vessel_data)
    axes(handles.axesImage);
    handles.vessel_data.update_image_lines(handles.axesImage);
end
...
global list_of_vessel_segments_to_mark;
list_of_vessel_segments_to_mark = [];
% Get image filter
...
axes(handles.axesImage);
set(handles.axesImage , 'DrawMode' , 'fast');
imshow(handles.vessel_data.im,'parent',handles.axesImage);
% Update painted lines
update_image_lines(handles.vessel_data, handles.axesImage);
...
% --- Executes when user attempts to close fig_QUARTZ_VESSEL_ANALYSIS.
function fig_QUARTZ_VESSEL_ANALYSIS_CloseRequestFcn(hObject, eventdata, handles)
...
function edit_AV_Callback(hObject, eventdata, handles)
% hObject    handle to edit_AV (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit_AV as text
%        str2double(get(hObject,'String')) returns contents of edit_AV as a double


% --- Executes during object creation, after setting all properties.
function edit_AV_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_AV (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
% --- Executes on button press in btnAssignAV.
function btnAssignAV_Callback(hObject, eventdata, handles)
% hObject    handle to btnAssignAV (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global list_of_vessel_segments_to_mark;
AorV = get( handles.edit_AV , 'String');
vd = handles.vessel_data;
if(isempty(vd))
    msgbox('vessel data is empty');
else
    %currentvessel = vd.selected_vessel;
    %curVesselIdx = vd.selected_vessel_ind;
    %vd.vessel_list(curVesselIdx).AV = AorV;
    idx = list_of_vessel_segments_to_mark;
    for ii  = 1: length(idx)
        nIdx = idx(ii);
        vd.vessel_list(1,nIdx).AV = str2num(AorV);
    end
    list_of_vessel_segments_to_mark = [];
end


% --- Executes on button press in btnSaveAV.
function btnSaveAV_Callback(hObject, eventdata, handles)
% hObject    handle to btnSaveAV (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
vd = handles.vessel_data;
...
copyvd= vd;
kdx = [];
for idx = 1:length(vd.vessel_list)
    if(~isempty(vd.vessel_list(idx).AV))
        kdx(end+1) = idx;
        
    end
end
%copyvd.trim_vessels(kdx);
save(['C:\D\Quartz\EPIC-A-v\' vd.file_name '.mat'] , 'copyvd');


function seedclick_short(src,eventdata, handles)
    global list_of_vessel_segments_to_mark;
    
    mouse=get(handles.axesImage ,'CurrentPoint');
    r_rowNum = round(mouse(1,2)); r_colNum = round(mouse(1,1));
    vd = handles.vessel_data;
    img_size  = size(vd.im_orig);
    if(r_rowNum > img_size(1) || r_rowNum < 0 || r_colNum > img_size(2) || r_colNum < 0)
        return
    end
    selIdx = vd.segment_label(r_rowNum, r_colNum);
    if(selIdx > 0  && selIdx < numel(vd.vessel_list) )
        vd.selected_vessel_ind = selIdx;
        ang = vd.selected_vessel.der;
        th = (rad2deg(atan2(ang(:,1),ang(:,2))))*-1;
        %vd.selected_vessel = vd.vessel_list(vd.selected_vessel_ind);
        % Update the drop-down list
        update_drop_down_list(handles , false);
        % Update the other data
        update_diameters_list(handles,false);
        update_vessel_table(handles);
        list_of_vessel_segments_to_mark = [list_of_vessel_segments_to_mark selIdx];
    end


% --- Executes on button press in btnOpenFig.
function btnOpenFig_Callback(hObject, eventdata, handles)
% hObject    handle to btnOpenFig (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
%loadAvGT();
LoadAV;
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
delete(handles.fig_QUARTZ_VESSEL_ANALYSIS);
...
%     btnZoomIn = findall(a,'ToolTipString', 'Zoom In');
%     if( btnZoomIn )
%         set(btnZoomIn , 'Separator' , 'off' , 'OffCallback' , {@zoomInCallback,1,currFigure} , 'OnCallback' ,{@zoomInCallback,2,currFigure});
%     end
%     
%     btnZoomOut = findall(a,'ToolTipString', 'Zoom Out');
%     if( btnZoomOut )
%         set(btnZoomOut , 'OffCallback' , {@zoomOutCallback,1,currFigure} , 'OnCallback' ,{@zoomOutCallback,2,currFigure});
%     end
%     btnPan = findall(a,'ToolTipString', 'Pan');
%     if( btnPan )
%         set(btnPan , 'OffCallback' , {@panCallback,1,currFigure} , 'OnCallback' ,{@panCallback,2,currFigure});
%     end
    clear a; clear b;


% --- Executes when selected object is changed in btnGrpImageSelection.
function btnGrpImageSelection_SelectionChangeFcn(hObject, eventdata, handles)
% hObject    handle to the selected object in btnGrpImageSelection 
...
function DisplayImage(handles)
    ...
    if get(handles.rb_RGB, 'value')
        imshow(handles.vessel_data.im_orig,'parent',handles.axesImage);
    else
        imshow(handles.vessel_data.im,'parent',handles.axesImage);
    end
