function [vessels, dist_max , sls_lab] = thinned_vessel_segments(bw, spur_length, min_px, remove_extreme, clear_branches_dist)
...
[bw_segments, dum, dist_trans , sls] = binary_to_thinned_segments(bw, spur_length, clear_branches_dist);
...
if(cc.NumObjects ~= length(cc.PixelIdxList) )
    msgbox('oh what should i do now');
end
...
for ii = cc.NumObjects:-1:1
    ...
    if numel(cc.PixelIdxList{ii}) <= 2
        ...
        vessels(ii).orig_centre = [row, col];
        vessels(ii).vesID = ii;
        continue;
    end
    ...
    %vessels(ii).orig_centre = [row(locs), col(locs)];
    vessels(ii).orig_centre = vessels(ii).centre ;
    vessels(ii).vesID = ii;
end
...
if nargout > 1
 ...
 %temp moazam - start
label = zeros(size(bw));
log_zeros = logical(label);
sls_lab = label;
% cent_p = logical(label);

iid_remove_inds = find(remove_inds == 1);

limit = cc.NumObjects-numel(iid_remove_inds);
 for kk=1:limit
%      if(~isempty(find(iid_remove_inds == kk)))
%          continue;
%      end
     cent_p = log_zeros;
     label(cc.PixelIdxList{kk}) = kk;
     cent_p(cc.PixelIdxList{kk})= 1;
     sls_p = imreconstruct(cent_p, sls) ;
     sls_lab(sls_p) = kk;
 end
%temp moazam - end
 
% sls = imreconstruct(cent_seg, sls);
% 
% for jj = 1 : length(cc.PixelIdxList)
%     label(cc.PixelIdxList{jj}) = jj;
% end
% a=1;
