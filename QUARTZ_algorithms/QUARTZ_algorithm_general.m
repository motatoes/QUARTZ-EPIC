function [vessel_data, args, cancelled] = QUARTZ_algorithm_general(vessel_data, args, prompt)
...
%seg_img_path = ['C:\D\Quartz\Output\LSI_DTH\Segmented\' vessel_data.file_name '\' vessel_data.file_name '.jpg'];
%seg_img_path = ['C:\D\Quartz\TEST\Seg\' vessel_data.file_name '.jpg']
%c = imread(seg_img_path);
%c = im2bw(c, graythresh(c));

%im_orig_path =['C:\D\Quartz\EPIC\AnisoOnPreHomo\' vessel_data.file_name];
%load([im_orig_path '_aniso-on-ph.mat']);
im = vessel_data.im_orig(:,:,2);
im = adapthisteq (im);
vessel_data.im = im;

%vessel_data.bw = logical(c);
